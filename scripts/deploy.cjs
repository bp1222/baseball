...
const getCloudFormationOuputValue = (key) => {
    ...
    aws cloudformation describe-stacks \
        --stack-name baseballseries-stack \
        --no-paginate \
        --no-cli-pager \
        --output text \
...
const uploadFiles = () => {
    ...
    const s3BucketName = getCloudFormationOuputValue("WebsiteBucketName");
...
const clearCloudFrontCache = () => {
    const distributionId = getCloudFormationOuputValue("DistributionId");
