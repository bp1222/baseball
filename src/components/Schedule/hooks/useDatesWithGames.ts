...
export const useDatesWithGames = (): UseDatesWithGamesResult => {
  ...
  const sortedDates = useMemo(() => {
    ...
    seasonSeries.forEach((s) => s.games.forEach((g) => set.add(dayjs(g.gameDate).format('YYYY-MM-DD'))))
    return Array.from(set)
      .sort()
      .map((d) => dayjs(d).startOf('day'))
  }, [seasonSeries])

  const datesSet = useMemo(() => new Set(sortedDates.map((d) => d.format('YYYY-MM-DD'))), [sortedDates])
