...
export const GetBadgeColors = (type: SeriesType, result: SeriesResult, mode: ThemeMode = 'light'): ResultColors => {
  const dark = mode === 'dark'
  if (type === SeriesType.World && result === SeriesResult.Win) {
    return dark ? { background: '#2e2a1a', border: '#4a4230' } : { background: amber[400], border: amber[700] }
  }
